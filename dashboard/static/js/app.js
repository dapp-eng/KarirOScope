(function () {
  ...
  function buildMerged(result) {
    ...
    return Object.assign({}, _data, {
      ...
      gap_analysis: (result.gap_analysis || []).length
        ? result.gap_analysis
        : _data.gap_analysis || [],
      ...
      data_status: Object.assign({}, _data.data_status || {}, {
        ...
        recommendations:
          (result.recommendations || []).length > 0 || (_data.data_status || {}).recommendations,
      }),
    });
  }
          ...
          [
            {
              ...
              orientation: 'h',
              marker: {
                color: sorted.map((r) => r.avg_salary),
                colorscale: [
                  [0, COLORS.cyan + '88'],
                  [1, COLORS.purple],
                ],
  ...
  function navigate(section) {
    ...
    if (sec) {
      ...
      sec
        .querySelectorAll(
          '.kpi-card, .chart-card, .rec-card, .section-banner, .scanner-form-card, .job-table-card',
        )
        .forEach((elem, i) => {
  ...
  function updateSidebarMeta(data) {
    ...
    if (statusRow) {
      ...
      statusRow.innerHTML = [
        ...
        .map((i) => `<div class="status-dot ${st[i.k] ? '' : 'missing'}">${i.l}</div>`)
        .join('');
    }
  }
  function renderKpis(kpis, containerId) {
    ...
    const cards = [
      {
        icon: `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 13V6a2 2 0 00-2-2H5a2 2 0 00-2 2v14l4-4h12a2 2 0 002-2z"/></svg>`,
        value: fmt(kpis.total_jobs),
        label: 'Total Job Postings',
        sub: kpis.location_query ? 'in ' + kpis.location_query : '',
        c: COLORS.blue,
        c2: COLORS.cyan,
      },
      {
        icon: `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 21h18M9 8h1m-1 4h1m-1 4h1M14 8h1m-1 4h1m-1 4h1M5 21V5a2 2 0 012-2h10a2 2 0 012 2v16"/></svg>`,
        value: fmt(kpis.unique_companies),
      ...
      },
      {
        ...
        value: kpis.top_category ? truncate(kpis.top_category, 16) : '-',
        label: 'Top Job Category',
        noAnim: true,
        sub: kpis.top_category_count ? fmt(kpis.top_category_count) + ' postings' : '',
        c: COLORS.teal,
        c2: COLORS.green,
      },
      {
        ...
        value: kpis.date_range || '-',
        label: 'Date Range',
        noAnim: true,
        sub: kpis.run_timestamp
          ? 'Updated ' + String(kpis.run_timestamp).split('T')[0].split(' ')[0]
          : '',
        c: COLORS.amber,
        c2: COLORS.red,
      },
  ...
  function renderIgKpis(kpis) {
    ...
    const cards = [
      ...
      },
      {
        ...
        value: kpis.ig_avg_er !== undefined ? safePct(kpis.ig_avg_er) : '-',
  ...
  function renderBanner(kpis, isLive) {
    ...
    if (tag) {
      tag.textContent = isLive ? 'Live Scan Active' : 'Notebook Dataset';
      tag.className = 'banner-tag' + (isLive ? ' live' : '');
    }
    if (title) {
    ...
    if (stats) {
      ...
      stats.innerHTML = items
        .map(
          (i) =>
            `<div class="banner-stat"><div class="banner-stat-val">${i.val}</div><div class="banner-stat-label">${i.label}</div></div>`,
        )
        .join('');
    }
  }
  ...
  function chartCategories(data) {
    ...
    if (!cats.length) {
      if (el('chart-categories')) el('chart-categories').innerHTML = noDataHtml();
      return;
    }
    renderChart(
      'chart-categories',
  ...
  function chartLocations(data) {
    ...
    if (!locs.length) {
      if (el('chart-locations')) el('chart-locations').innerHTML = noDataHtml();
      return;
    }
  ...
  function chartTrendsLine(data) {
    const trend = data.trend || [];
    if (!trend.length) {
      if (el('chart-trends-line')) el('chart-trends-line').innerHTML = noDataHtml();
      return;
    }
    const x = trend.map((r) => r.year_month || r.month || '');
    const y = trend.map((r) => r.posting_count || r.count || 0);
  ...
  function chartSkillsBar(data) {
    ...
    if (!skills.length) {
      if (el('chart-skills-bar')) el('chart-skills-bar').innerHTML = noDataHtml();
      return;
    }
    ...
    renderChart(
      ...
      {
        xaxis: {
          title: { text: 'Job Posting Mentions', standoff: 8 },
          range: [0, maxX * 1.22],
          automargin: true,
        },
  ...
  function chartCatSkills(data) {
    ...
    if (!rows.length) {
      if (el('chart-cat-skills')) el('chart-cat-skills').innerHTML = noDataHtml();
      return;
    }
    ...
    const map = {};
    rows.forEach((r) => {
      map[(r.job_category || '') + '||' + (r.skill || '')] = r.frequency || 0;
    });
    const topSkills = allSkills
      .sort(
        (a, b) =>
          cats.reduce((s, c) => s + (map[c + '||' + b] || 0), 0) -
          cats.reduce((s, c) => s + (map[c + '||' + a] || 0), 0),
      )
      .slice(0, 15);
  ...
  function chartCompanies(data) {
    ...
    if (!companies.length) {
      if (el('chart-companies')) el('chart-companies').innerHTML = noDataHtml();
      return;
    }
    ...
    renderChart(
      ...
      {
        xaxis: {
          title: { text: 'Job Postings', standoff: 8 },
          range: [0, maxX * 1.22],
          automargin: true,
        },
        yaxis: { autorange: 'reversed', tickfont: { size: 11 }, automargin: true },
  ...
  function chartIgType(ig) {
    ...
    const erCol =
      rows[0].avg_engagement_rate !== undefined ? 'avg_engagement_rate' : 'engagement_rate';
  ...
  function chartIgDay(ig) {
    ...
    const dayMap = {};
    rows.forEach((r) => {
      dayMap[r.day_of_week] = r.engagement_rate || 0;
    });
  ...
  function renderRecommendations(data) {
    ...
    container.innerHTML = sorted
      .map((r, i) => {
        ...
        const keywords = (r.top_keywords || r.top_keywords_in_jobs || '')
          .split(',')
          .slice(0, 4)
          .map((k) => k.trim())
          .filter(Boolean);
  ...
  function drawJobTable() {
    ...
    tbody.innerHTML = rows
      .map((job, i) => {
        ...
        const catColor = {
          'Data and Technology': '#0A66C2',
          'Software Engineering': '#7C3AED',
          'Marketing and Content': '#EC4899',
          'Sales and Business Development': '#F59E0B',
          'People and HR': '#0D9488',
          'Finance and Operations': '#10B981',
          'Product and Design': '#EF4444',
        };
        const catStyle = catColor[cat]
          ? `background:${catColor[cat]}18;border-color:${catColor[cat]}30;color:${catColor[cat]}`
          : '';
  ...
  function filterJobs(query) {
    ...
    let base = _jobsAll;
    if (_remoteFilter === 'remote')
      base = _jobsAll.filter((j) => (j.is_remote || '').toLowerCase() === 'true');
    else if (_remoteFilter === 'onsite')
      base = _jobsAll.filter((j) => (j.is_remote || '').toLowerCase() === 'false');
    if (!q) {
      _jobsFiltered = [...base];
    } else {
      _jobsFiltered = base.filter(
        (j) =>
  ...
  function onScanDone(result) {
    ...
    _scanResult = result;
    try {
      localStorage.setItem(LS_SCAN_KEY, JSON.stringify(result));
    } catch (e) {}
  ...
  function renderScanCharts(result) {
    ...
    if (result.skills && result.skills.length) {
      ...
      renderChart(
        ...
        [
          {
            y: top.map((r) => r.skill || ''),
            x: top.map((r) => r.frequency || 0),
            type: 'bar',
            orientation: 'h',
            marker: {
              color: top.map((r) => r.frequency || 0),
              colorscale: [
                [0, COLORS.cyan + '88'],
                [1, COLORS.blue],
              ],
    ...
    if (result.companies && result.companies.length) {
      ...
      renderChart(
        'chart-scan-companies',
        [
          {
            y: top.map((r) => r.company_name),
            x: top.map((r) => r.job_count),
            type: 'bar',
            orientation: 'h',
  ...
  function initTypingAnimation() {
    ...
    function tick() {
      pauseTimer = setTimeout(type, isDeleting ? 30 : 52);
    }
    tick();
  }
  ...
  function triggerEasterEgg() {
    ...
    if (brand) {
      brand.classList.remove('easter-active');
      brand.offsetHeight;
      brand.classList.add('easter-active');
      setTimeout(() => brand.classList.remove('easter-active'), 800);
    }
  ...
  function renderScanExtras(result) {
    ...
    if (salary.length >= 1 && salaryCard) {
      ...
      if (salaryMeta)
        salaryMeta.textContent = `Avg ${cur} ${avg} | ${salaryStats.pct_disclosed || 0}% jobs disclosed`;
      const sorted = [...salary].sort((a, b) => b.avg_salary - a.avg_salary);
      const maxSal = Math.max(...sorted.map((r) => r.avg_salary));
      renderChart(
        'chart-scan-salary',
  ...
  function checkAlertThreshold(kpis, skills) {
    ...
    if (condition === 'gt' && totalJobs > parseInt(rawVal)) {
      ...
      if (found) {
        showNotification(
          `Alert: Skill "${found.skill}" is in the Top 10 scan results (${found.frequency} mentions)`,
          'success',
          8000,
        );
      } else {
        showNotification(
          `Alert: Skill "${rawVal}" was not found in the Top 10 scan results`,
          'warning',
          8000,
        );
      }
  ...
  function loadMySkills() {
    try {
      _mySkills = JSON.parse(localStorage.getItem(LS_SKILLS_KEY) || '[]');
    } catch (e) {
      _mySkills = [];
    }
    renderMySkills();
  ...
  function saveMySkills() {
    try {
      localStorage.setItem(LS_SKILLS_KEY, JSON.stringify(_mySkills));
    } catch (e) {}
  }
  ...
  function renderMySkills() {
    ...
    if (countEl)
      countEl.textContent =
        _mySkills.length + ' skill' + (_mySkills.length !== 1 ? 's' : '') + ' added';
    if (actions) actions.style.display = _mySkills.length ? 'flex' : 'none';
    if (!container) return;
    container.innerHTML = _mySkills
      .map(
        (s) => `
  ...
  window._removeSkill = function (skill) {
    removeMySkill(skill);
  };
  function analyzeSkillGap(marketSkills) {
    ...
    `;
    showNotification(
      `Skill Gap Analysis completed. Coverage: ${pct}%`,
  ...
  function initEventListeners() {
    ...
    el('scanKeyword').addEventListener('keydown', (e) => {
    ...
    if (skillGapInput && btnAddSkill) {
      skillGapInput.addEventListener('keydown', (e) => {
        if (e.key === 'Enter') {
          addMySkill(skillGapInput.value);
          skillGapInput.value = '';
        }
      });
    ...
    document.querySelectorAll('.remote-filter-btn').forEach((btn) => {
      btn.addEventListener('click', () => {
        document
          .querySelectorAll('.remote-filter-btn')
